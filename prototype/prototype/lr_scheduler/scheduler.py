...
class _LRScheduler(object):
    ...
    def step(self, this_iter=None):
        ...
        for param_group, lr in zip(self.optimizer.param_groups, self._get_new_lr()):
            if not getattr(param_group, 'lr_not_decayed', False):
                param_group['lr'] = lr
...
class SimSiamLRScheduler(CosineLRScheduler):
    def __init__(self, optimizer, warmup_epochs, warmup_lr, num_epochs, base_lr, final_lr, iter_per_epoch):
        max_iter = num_epochs * iter_per_epoch
        min_lr = final_lr
        warmup_steps = warmup_epochs * iter_per_epoch
        warmup_lr = base_lr if warmup_steps == 0 else warmup_lr

        super(SimSiamLRScheduler, self).__init__(optimizer, max_iter, min_lr, base_lr, warmup_lr, warmup_steps, last_iter=0)


class SimSiamStepLRScheduler(StepLRScheduler):
    def __init__(self, optimizer, lr_steps, lr_mults, warmup_epochs, warmup_lr, num_epochs, base_lr, iter_per_epoch):
        max_iter = num_epochs * iter_per_epoch
        warmup_steps = warmup_epochs * iter_per_epoch
        lr_steps = [e * iter_per_epoch for e in lr_steps]
        warmup_lr = base_lr if warmup_steps == 0 else warmup_lr

        super(SimSiamStepLRScheduler, self).__init__(optimizer, lr_steps, lr_mults, base_lr, warmup_lr, warmup_steps, max_iter, last_iter=0)


SimSiamLR = SimSiamLRScheduler
SimSiamStep = SimSiamStepLRScheduler
