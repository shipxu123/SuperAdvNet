...
from ..backbone.bignas_resnet_bottleneck_subnet import bignas_resnet_bottleneck


big_resnet_bottleneck_kwargs = {
    'out_channel': {
        'space': {
...
class Big_ResNet_Bottleneck(Bignas_SearchSpace):
    ...
    def __init__(self,
                 ...
                 # search space
                 **kwargs):
        super(Big_ResNet_Bottleneck, self).__init__(**kwargs)
        ...
        _block_index = 1
        self.stage_out_idx = []
        for s, act_func, n_block in zip(self.stride_stages[1:5], self.act_stages[1:5], self.depth[1:5]):
            ...
            for i in range(n_block):
                ...
                mobile_inverted_conv = DynamicBottleneckBlock(
                    ...
                    KERNEL_TRANSFORM_MODE=kernel_transform, divisor=self.divisor)
    ...
    def build_active_subnet(self, subnet_settings):
        subnet = bignas_resnet_bottleneck(act_stages=self.act_stages, stride_stages=self.stride_stages,
                                          use_maxpool=self.use_maxpool, dropout_rate=self.dropout_rate,
...
def big_resnet_bottleneck(**kwargs):
    return Big_ResNet_Bottleneck(**kwargs)


if __name__ == '__main__':
    import torch
    from prototype.spring.models import SPRING_MODELS_REGISTRY
    SPRING_MODELS_REGISTRY.register('big_resnet_bottleneck', big_resnet_bottleneck)

    cls_model = SPRING_MODELS_REGISTRY['big_resnet_bottleneck']
    det_model = SPRING_MODELS_REGISTRY['big_resnet_bottleneck'](
