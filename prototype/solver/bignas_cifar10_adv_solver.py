...
class BigNASCIFARAdvSolver(BaseSolver):
    ...
    def data_epoch2iter(self, cfg_dataset):
        ...
        dataset = CIFAR10(root='/mnt/lustre/share/prototype_cifar/cifar10/',
                                        train=True,
                                        download=False)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(0, self.config.data.max_epoch)):
            ...
            for i, (input, target) in enumerate(self.train_data['loader']):
                ...
                # compute and update gradient
                # 这个地方由于bignas的训练方式会产生一些没有梯度，但是required gradient的地方
                # 所以在没有采样max的大网络下，简单的sync_gradient可能会碰到grad None但required gradient=True的情况
    ...
    def evaluate_subnet(self):
        ...
        # self.logger.info('Subnet:\n{}'.format(self.model.module.backbone.module.module_str))
...
def main():
    ...
    # evaluate or train
    if args.phase in ['evaluate_subnet', 'evaluate_max_adv_subnet', 'finetune_subnet', 'sample_accuracy']:
