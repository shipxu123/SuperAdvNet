...
from .normal_blocks import ConvBlock, LinearBlock, BasicBlock, get_same_length


__all__ = ['bignas_resnet_basic']


class BigNAS_ResNetD_Basic(nn.Module):

    def __init__(self,
                 # search settings
                 out_channel=[32, 32, 64, 64, 128, 256, 512],
                 depth=[1, 1, 1, 2, 2, 2, 2],
        ...
        super(BigNAS_ResNetD_Basic, self).__init__()
        ...
        self.third_conv = ConvBlock(
            in_channel=self.out_channel[1], out_channel=self.out_channel[2], kernel_size=self.kernel_size[2],
            stride=stride_stages[2], act_func=act_stages[2], NormLayer=NormLayer)
        ...
        _block_index = 3
        self.stage_out_idx = []
        for s, act_func, n_block, output_channel in zip(self.stride_stages[3:], self.act_stages[3:], self.depth[3:],
                                                        self.out_channel[3:]):
...
def bignas_resnetd_basic(**kwargs):
    return BigNAS_ResNetD_Basic(**kwargs)


if __name__ == '__main__':
    from prototype.spring.models import SPRING_MODELS_REGISTRY
    SPRING_MODELS_REGISTRY.register('bignas_resnet_basic', bignas_resnetd_basic)

    cls_model = SPRING_MODELS_REGISTRY['bignas_resnetd_basic']()
    det_model = SPRING_MODELS_REGISTRY['bignas_resnetd_basic'](
